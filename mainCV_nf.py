...
def main():
    ...
    #
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(X_train)):
            X_train_fold, X_val_fold = torch.tensor(X_train[train_idx], dtype=torch.float32).to(device), torch.tensor(X_train[val_idx], dtype=torch.float32).to(device)
            y_train_fold, y_val_fold = torch.tensor(y_train[train_idx], dtype=torch.int64).to(device), torch.tensor(y_train[val_idx], dtype=torch.int64).to(device)
