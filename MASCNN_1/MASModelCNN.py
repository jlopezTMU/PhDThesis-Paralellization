...
class ProcessorAgent(Agent):
    def __init__(self, unique_id, model, X_train, y_train, X_test, y_test, device, args):
        ...
        self.total_examples_processed = len(X_test)  # Track total number of examples processed
    ...
    def step(self):
        ...
        self.fold_loss, self.fold_accuracy, self.neural_net_model, self.correct_classifications = train_simulated(
            self.unique_id, self.X_train, self.y_train, self.X_test, self.y_test, self.device, self.args
        )
