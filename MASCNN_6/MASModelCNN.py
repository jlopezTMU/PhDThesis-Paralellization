...
class ParallelizationModel(Model):
        ...
        if self.num_processors == 1:
            ...
            for i in range(self.num_processors):
                ...
                agent = ProcessorAgent(
                    ...
                    X_test=split_X_test[i],  # Assign the correct test split
                    y_test=split_y_test[i],  # Assign the correct test labels
                    device=self.device,
                    args=self.args
                )
                self.schedule.add(agent)
    ...
    def report_testing_accuracy(self):
        ...
        print(f"--- Testing Accuracy: {total_correct}/{total_examples} = {testing_accuracy:.2f}% ---")
