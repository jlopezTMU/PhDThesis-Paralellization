...
def print_memory_usage():
    ...
    print("Memory consumption:", process.memory_info().rss / 1024 ** 2, "MB")
...
#train
def train(rank, args, model, device, train_dataset, dataloader_kwargs, val_dataloader_kwargs, results, model_states, start_time):
...
if __name__ == '__main__':
    ...
    print("This is the memory usage at start of program:", print_memory_usage())
    ...
    for rank in range(args.num_processes):
        ...
        p = mp.Process(target=train, args=(rank, args, cpu_model, device, train_dataset, dataloader_kwargs, val_dataloader_kwargs, results, model_states, start_time))
   ...
   # Calculate global_avg_accuracy before the loop
    global_avg_accuracy = sum(result[2] for result in results) / len(results)
    best_global_accuracy = (0.0, -1, 0.0, 0.0) # Initialize to any reasonable value
    best_processor = -1
    best_model_state = None
    ...
    for i, (accuracy, fold, average_accuracy, training_time) in enumerate(results):
         ...
         if abs(average_accuracy - global_avg_accuracy) < abs(best_global_accuracy[2] - global_avg_accuracy):
             best_global_accuracy = (accuracy, fold, average_accuracy, training_time)
             best_processor = i
             best_model_state = {k: v.cpu() for k, v in cpu_model.cpu().state_dict().items()}
    ...
    print("--- This is the memory usage at the end of program:", print_memory_usage())
