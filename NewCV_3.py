...
def train(fold, train_idx, val_idx):
    ...
    # Progress message
    print(f"PID {os.getpid()}: Processed data from indices {train_idx[0]} to {train_idx[-1]}, Fold {fold+1}, Accuracy: {val_accuracy*100:.2f}%")
...
def parallel_kfold():
    ...
    with Pool(processes=args.processors) as pool:
        results = pool.starmap(train, [(fold, train_idx, test_idx) for fold, (train_idx, test_idx) in enumerate(kf.split(X))])
    end_time = time.time()
...
def main():
    ...
    print(f"Cross-validation took {elapsed_time:.4f} seconds")
