...
class UADetracSceneDataset(Dataset):
    ...
    def __init__(self, csv_path, dataset_root, split, transform=None, limit=0):
        ...
        # Hard guard: dataset_root must be a directory (NOT the CSV)
        if self.dataset_root.is_file():
            raise ValueError(
                f"UA-DETRAC dataset_root points to a FILE, not a directory: {self.dataset_root}\n"
                f"You likely passed the CSV path as dataset_root by mistake.\n"
...
def build_loaders(Training_ds, Training_lbls, Testing_ds, Testing_lbls, device, args):
    ...
    if dataset_mode:
        # Training_lbls / Testing_lbls are index lists in dataset-mode
        train_subset = Subset(Training_ds, Training_lbls)
        test_subset  = Subset(Testing_ds,  Testing_lbls)

        # Keep the total number of workers bounded across processors.
    ...
    # Keep your existing array-mode evaluation approach
    test_loader = None
...
def get_model(arch, num_classes=10, pretrained=False):
    ...
    if arch == 'ResNet18':
        print(f"[DEBUG] ResNet18 pretrained={pretrained}, weights={'DEFAULT' if pretrained else 'None'}")
...
def train_simulated(
    ...
    if train_loader is None:
        ...
        # If loaders are passed by the caller, dataset_mode is inferred from test_loader.
        if dataset_mode is None:
            dataset_mode = (test_loader is not None)

    if not dataset_mode:
        # In array mode, evaluation tensors are stored in array_eval.
