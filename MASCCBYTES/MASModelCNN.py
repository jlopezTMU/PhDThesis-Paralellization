...
class ParallelizationModel(Model):
    ...
    def step(self):
        ...
        # Estimate model size (in bytes) from the first agent with a model
        for agent in self.schedule.agents:
            if agent.neural_net_model is not None:
                if isinstance(agent.neural_net_model, dict):
                    model_state_dict = agent.neural_net_model
                else:
                    model_state_dict = agent.neural_net_model.state_dict()
                model_size = sum(v.element_size() * v.nelement() for v in model_state_dict.values())
                print(f"Estimated Model Size per Node: {model_size} bytes")
                break
