...
# note that train module remains as trainCV3
# This is the main program mainCV3.py
#######################################################
...
from trainCV3 import train, test
...
###
    best_fold = -1
    best_models = []
    # Iterate through results to find the model with the best accuracy and shortest time
    best_model_index = -1
    best_accuracy = 0.0
    best_time = 0.0
    for i, (accuracy, time) in enumerate(results):
        if accuracy > best_accuracy or (accuracy == best_accuracy and time < best_time):
            best_accuracy = accuracy
            best_time = time
            best_model_index = i

    print("The model with the highest accuracy {:.2f}% took {:.2f} seconds to train.".format(best_accuracy, best_time))

    # Once training is complete, we can test the model
    model.load_state_dict(model_states[best_model_index])
    test(args, model, device, dataset2, dataloader_kwargs)
    print('DEBUG time=',time)
    print('DEBUG init_time=',init_time)
