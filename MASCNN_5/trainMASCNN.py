...
def train_simulated(fold, train_idx, val_idx, X, y, device, args, original_training_size, sync_callback):
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            correct_predictions = (val_outputs.argmax(1) == y_val_fold).sum().item()
            validation_accuracy = (correct_predictions / len(y_val_fold)) * 100

            # Debug: Print validation results
            print(f"Epoch {epoch+1}, Validation Loss: {val_loss:.4f}, Validation Accuracy: {validation_accuracy:.2f}%")
