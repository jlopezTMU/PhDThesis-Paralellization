...
def make_dirichlet_uadetrac_indices(labels, num_processors, alpha=0.5, seed=42):
    ...
    for attempt in range(max_attempts):
        ...
        for agent_id in range(num_processors):
            ...
            if np.min(class_counts) < 50:
                valid = False
                break

        if valid:
            print(
                f"Accepted non-IID UA-DETRAC partition "
            ...
            for agent_id in range(num_processors):
                ...
                print(
                    f"Node {agent_id + 1} non-IID UA-DETRAC class distribution: "
    ...
    raise RuntimeError(
        "Could not create a valid non-IID UA-DETRAC partition after "
