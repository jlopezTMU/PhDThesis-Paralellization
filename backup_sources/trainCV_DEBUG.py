...
def train(fold, train_idx, val_idx, X, y, device, args):
    ...
    # Data is split
    print(f"DATA SPLIT PID {os.getpid()}, Fold {fold+1}, Accuracy: {val_accuracy*100:.2f}%, Elapsed time: {elapsed_time:.4f} seconds")
    ## NO!print(f"PID {os.getpid()} of {processor}: Processed data from indices {train_idx[0]} to {train_idx[-1]}, Fold {fold+1}, Accuracy: {val_accuracy*100:.2f}%, Elapsed time: {elapsed_time:.4f} seconds")



    return val_loss, val_accuracy, model.state_dict()  # Return the model as well
