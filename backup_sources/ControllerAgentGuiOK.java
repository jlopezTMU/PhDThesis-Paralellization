...
/* JL 20240730*/
private void runMainCVDivFolds() {
    try {
        ...
        // Create a thread to read from the input stream and display the information in the JTextArea
        new Thread(() -> {
            String line;
            try {
                while ((line = reader.readLine()) != null) {
                    appendToResultArea(line); // Append the line to the JTextArea
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
        ...
        if (exitCode == 0) {
            appendToResultArea("mainCV_divFolds.exe executed successfully.");
        } else {
            appendToResultArea("mainCV_divFolds.exe execution failed with exit code " + exitCode);
        }
