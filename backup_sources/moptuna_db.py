...
# This version tries only 10 folds HP
# This version introduces several optimizers as hyperparameters
...
def optimize_hyperparameters(trial, X, y, device, args):
    ...
    optimizer_name = trial.suggest_categorical('optimizer', ['Adam', 'SGD', 'RMSprop', 'Adagrad', 'AdamW', 'Adadelta'])
    ...
    epochs = trial.suggest_int('epochs', 5, 10)
    folds = trial.suggest_int('folds', 5, 10) ##categorical?
    ## processors = trial.suggest_int('processors', 2, 8)  Commented, no interested in varying it

    # Print the current hyperparameters
    print ("--------------------------------------------")
    print(f"@DEBUG CURRENT HP USED BY OPTUNA: Trial {trial.number} hyperparameters: batch_size={batch_size}, lr={lr}, epochs={epochs}, folds={folds}")
    ...
    ## args.processors = processors  # Updating the processors Commented, no interested in varying it

    ##global total_processing_time

    # Use the updated args in the training function
    results, processing_time = parallel_kfold(X, y, device, args, optimizer_name)
...
##    processing_time = time.time() - start_time
    ...
    ## NO USEprint("@Debug Partial TPT: %.6f, and PT: %.6f" % (total_processing_time, processing_time))
    ...
    penalty_factor = 0.0001  # Adjust based on how much you want to penalize time
    composite_score = average_accuracy - (penalty_factor * processing_time)
    print("@DEBUG **** composite_score: ", {composite_score}, " average_accuracy: ", {average_accuracy}, " processing_time: ", {processing_time})
    return composite_score

def parallel_kfold(X, y, device, args, optimizer_name):
    ...
    with Pool(processes=args.processors) as pool:
        results = pool.starmap(train, [(fold, train_idx, val_idx, X, y, device, args, optimizer_name) for fold, (train_idx, val_idx) in enumerate(kf.split(X))])
...
# Optuna study for hyperparameter optimization
    ...
    # Define fixed hyperparameters so Optuna don't initialized them randomlyi! I am not using this!!
    fixed_hyperparams = {
        'batch_size': 64,
        'lr': 0.01,
        'epochs': 10,
    ...
    print("HP are being initialized to these values:",fixed_hyperparams)
    # Create a Fixed Trial
    #fixed_trial = optuna.trial.FixedTrial(fixed_hyperparams)

    # Run the trial using your objective function
    ## fixed_trial_result = optimize_hyperparameters(fixed_trial, X_train, y_train, device, args)

    study = optuna.create_study(
    direction='maximize',
    study_name='myresearch_study_optimizer',  # Name your study
    storage='postgresql://jlopez:dbpass@localhost/optuna_study',  # Use your PostgreSQL credentials and database name
    load_if_exists=True)

    #fixed_trial_result = optimize_hyperparameters(trial, X_train, y_train, device, args) ## declared after study creation to take into account initialization
    ...
    print(f"Total processing time for all trials:", total_processing_time)
    ...
    ## args.processors = study.best_params['processors']  # Update processors NOT interested
    #== OPTUNA ENDS
    ...
    #
        print(f"*** Total processing time for all folds across for all {args.processors} processors\
              is {total_elapsed_time:4f} seconds \
