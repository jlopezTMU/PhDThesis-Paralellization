...
import sys
import logging
from datasets import load_dataset  # Added for loading the QNLI dataset
...
def train_simulated(fold, train_idx, val_idx, X, y, device, args):
    # Load the QNLI dataset and cache it in the data subdirectory
    ...
    y_train_fold = torch.tensor([y[i] for i in train_idx], dtype=torch.long).to(device)
    y_val_fold = torch.tensor([y[i] for i in val_idx], dtype=torch.long).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for i, _ in enumerate(range(0, len(train_encodings['input_ids']), args.batch_size)):
            ...
            predicted_labels = outputs.logits.argmax(dim=1)  # Get the predicted class
    ...
    with torch.no_grad():
        with autocast():
            ...
            val_accuracy = ((val_outputs.logits.argmax(dim=1) == y_val_fold).float().mean().item())
