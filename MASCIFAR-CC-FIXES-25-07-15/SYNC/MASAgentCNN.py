...
class ProcessorAgent(Agent):
    ...
    def set_weights_from_averaged_state_dict(self, avg_state_dict):
        """
        Loads averaged weights (float/int tensors handled safely) into the agent's local model.
        Should be called by the global synchronizer after averaging.
        """
        model_state = self.neural_net_model.state_dict()
        new_state_dict = {}
        for k, v in model_state.items():
            avg_v = avg_state_dict[k]
            # Safely handle float/integer tensor casting.
            if v.dtype in (torch.float32, torch.float64, torch.float16):
                new_state_dict[k] = avg_v.to(v.dtype)
            else:
                new_state_dict[k] = avg_v.to(v.dtype)
        self.neural_net_model.load_state_dict(new_state_dict)
